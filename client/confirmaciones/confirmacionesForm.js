...
.controller("ConfirmacionesCtrl", ConfirmacionesCtrl);
 function ConfirmacionesCtrl($scope, $meteor, $reactive, $state, toastr, $stateParams){
  ...
  if($stateParams.id != undefined){
	...
	this.helpers({
 	  objeto : () => {
		  	var confirmado = Confirmaciones.findOne($stateParams.id);
		  	
		  	if (confirmado != undefined)
		  	{	
			  		var user = Meteor.users.findOne(Meteor.userId());
 			
						if (confirmado.tipoSacerdote == "PARROQUIA")
						{
						...
						else if (confirmado.tipoSacerdote == "DIOCESIS")
						{
 						...
 						if (confirmado.tipoSacerdoteParroco != undefined)
 						{
	 							if (confirmado.tipoSacerdoteParroco == "PARROQUIA")
								{
 						...
 						{
	 							confirmado.tipoSacerdoteParroco = "OTRO";
 						}
 						
 						if (confirmado.estatus == "confirmado")
						{
								rc.estatus = false;
						}
						else if (confirmado.estatus == "enTramite")
						{
								rc.estatus = true;
 						}			
  ...
  this.guardar = function(objeto,form)
	{
		if (form.$invalid){
		  ...
		  if (objeto.apellidoPaterno == "" && objeto.apellidoMaterno == "")
			...
			loading(true);
			Meteor.call ("setServicios", objeto, "Confirmación", function(error,result){
				...
				if (result)
				{
				 		...
				 		$state.go('root.confirmacionesLista');
					 	loading(false);		 
	...
	this.actualizar = function(objeto,form)
	{
		  ...
		  if (objeto.apellidoPaterno == "" && objeto.apellidoMaterno == "")
			...
			Confirmaciones.update({_id:idTemp},{$set : objeto});
	...
	{
			if (rc.objeto.estatus == "confirmado")
			{
					rc.estatus = false;
			}
	...
	this.imprimirNotificacion = function(form)
	{
	      ...
	      reportNombre	: 'NotPConfirmacionOut',
		      type					: 'pdf',  
	      datos					: datos,
		    }, function(err, file) {
		      if(!err){
